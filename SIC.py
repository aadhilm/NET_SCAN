...
class NetworkScanner(tk.Tk):
        ...
        try:
            for i in range(1, end_ip + 1):
                ...
                if self.ping_device(target_ip):
                    ...
                    self.device_listbox.insert(tk.END, target_ip)
                    self.device_graph.add_node(target_ip)
                    self.device_graph.add_edge(f"{prefix}.1", target_ip)
                else:
    ...
    def run_traceroute(self):
        target_ip = self.device_listbox.get(tk.ACTIVE)
        if target_ip:
            self.append_output(f"Running traceroute to {target_ip}...\n")
            ans, unans = traceroute(target_ip, verbose=0)
            for snd, rcv in ans:
                self.append_output(f"{snd.src} -> {rcv.src}\n")
        else:
            self.append_output("Please select a device from the list.\n")
